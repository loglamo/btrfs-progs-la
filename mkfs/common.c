...
int make_btrfs_mp(int fd, struct btrfs_mkfs_config *cfg, int iter)
{
	...
	for (i = 0; i < blocks_nr; i++) {
		blk = blocks[i];
		cfg->blocks[blk] = system_group_offset + num_bytes*k + cfg->nodesize * i;
		total_used += cfg->nodesize;
	}
	...
	csum_tree_block_size(buf, btrfs_csum_type_size(cfg->csum_type), 0,
			     cfg->csum_type);
	ret = sbwrite(fd, buf->data, BTRFS_SUPER_INFO_OFFSET + BTRFS_SUPER_INFO_SIZE*k);
	if (ret != BTRFS_SUPER_INFO_SIZE) {
		ret = (ret < 0 ? -errno : -EIO);
		goto out;
	}
